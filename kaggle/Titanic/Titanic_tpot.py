# -*- coding: utf-8 -*-
"""
Created on Wed Mar 22 17:34:12 2017
...
# Load the data
input_df, submit_df = getDataSets(bins=True, scaled=True, \
                                               binary=True)
#submit_df=submit_df.drop(['Survived'],axis=1)
...
input_df_new=input_df.drop(['class'],axis=1).values
titanic_class = input_df['class'].values

#To begin our analysis, we need to divide our training data into training and validation sets
training_indices, validation_indices = training_indices, testing_indices =\
 train_test_split(input_df.index, stratify = titanic_class, train_size=0.70, test_size=0.30)
...
tpot.fit(input_df_new[training_indices], titanic_class[training_indices])

print tpot.score(input_df_new[validation_indices], input_df.loc[validation_indices, 'class'].values)

# Generate the predictions
submit_df_test=submit_df.values
submission = tpot.predict(submit_df_test)

# Create the submission file
final = pd.DataFrame({'PassengerId': submit_df['PassengerId'], 'Survived': submission})
final.to_csv('submission_new2.csv', index = False)
